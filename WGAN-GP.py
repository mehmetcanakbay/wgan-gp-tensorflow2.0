...
#This is the custom loss for our critic (discriminator). To be able to use more variables in our loss function, we make another function surrounding it.
def wass_loss_cr(model, batch, gen_img):
...
def train(gan, datas, batch_size, epochs=5):
    ...
    for epoch in range(epochs):
        ...
        for batch in data_list:
            #This for loop is to train the critic even more. Critic is being trained for 5 times.
            for _ in range(5):
...
critic.compile(loss=wass_loss_cr(critic, np.array(next(iterable_list)), generator(noise)), optimizer=tf.keras.optimizers.Adam(lr=1e-5, beta_1 = 0.0, beta_2=0.9))
